import React, { useEffect, useState } from "react";
import { useQuery, useMutation } from "react-apollo";
import MaterialTable from "material-table";
import AddBox from "@material-ui/icons/AddBox";
import CircularProgress from "@mui/material/CircularProgress";
import Box from "@mui/material/Box";
import { tableIcons } from "../../icons/tableIcon";
import { CREATEORS_REQUEST_LIST, CONFIRM_CREATORS } from "../../apollo/gql/gql";

const UserList = () => {
  const [selectedRow, setSelectedRow] = useState(null);
  const [userInfor, setUserInfor] = useState();
  // const [dates, setDates] = useState();
  // 크리에이터 요청 신청 리스트
  const { loading, error, data, refetch } = useQuery(CREATEORS_REQUEST_LIST, {
    fetchPolicy: "no-cache",
    variables: {
      condition: {
        where: "status=2",
        order_by: {
          field: "created_at",
          order: "DESC",
        },
        offset: 0,
        limit: 100,
      },
    },
  });

  // 크리에이터 승인creator
  const [creatorConfirm, { data: confirm, loading: dataLoading }] = useMutation(
    CONFIRM_CREATORS,
    {
      fetchPolicy: "no-cache",
      onCompleted(confirm) {
        if (confirm) {
          refetch();
        }
      },
    }
  );
  useEffect(() => {
    if (data) {
      setUserInfor(data.get_creators.map((item) => item));
    }
  }, [data]);

  const columns = [
    {
      title: "",
      field: "imageUrl",
      render: (rowData) => <img src="../img/userImg.png" alt="유저프로필" />,
    },

    {
      title: "닉네임",
      field: "user.nickname",
    },
    {
      title: "등급",
      field: "tier",
    },
    {
      title: "등록일",
      field: "created_at",
      render: (rowData) => (
        <h1 className="ok">
          <h1>{time(rowData.created_at)}</h1>
        </h1>
      ),
    },
    {
      title: "정보",
      field: "intro",
    },
  ];
  const time = (date1) => {
    var date = new window.Date(date1);
    var year = date.getFullYear();
    var month = "0" + (date.getMonth() + 1);
    var day = "0" + date.getDate();
    return year + "-" + month.substr(-2) + "-" + day.substr(-2) + " ";
  };

  if (loading) {
    <Box className="loadding">
      <CircularProgress />
    </Box>;
  }
  if (error) return "Error";

  return (
    <div>
      {dataLoading && (
        <Box className="loadding">
          <CircularProgress />
        </Box>
      )}
      <div className="productWrap">
        <MaterialTable
          //   style={{ width: "100%" }}
          title={"크리에이터 승인 "}
          columns={columns}
          icons={tableIcons}
          data={userInfor}
          onSelectionChange={(data) => setSelectedRow(data)}
          onRowClick={(event, rowData) => {
            // setToggle(!toggle);
            // setDatas(rowData);
          }}
          options={{
            sorting: false,
            searchFieldAlignment: "left",
            searchAutoFocus: true,
            paging: true,
            pageSize: 10,
            pageSizeOptions: [10, 20, 30, 40, 50],
            selection: true,
            showTextRowsSelected: false,
            // rowStyle: (rowData) => ({
            //   backgroundColor:
            //     selectedRow === rowData.tableData.id ? "#6ABAC9" : "#FFF",
            // }),
            // filtering: true,
            // actionsColumnIndex: -1,
          }}
          actions={[
            // {
            //   icon: () => (
            //     <Select
            //       labelId="demo-simple-select-label"
            //       id="demo-simple-select"
            //       style={{ width: 100 }}
            //       // value={age}
            //       // onChange={handleChange}
            //     >
            //       <MenuItem value={10}>All</MenuItem>
            //       <MenuItem value={20}>등록 요청대기</MenuItem>
            //       <MenuItem value={30}>수정 요청대기</MenuItem>
            //     </Select>
            //   ),
            //   tooltip: "상품 종류",
            //   isFreeAction: true,
            // },
            {
              icon: () => <AddBox></AddBox>,
              tooltip: "크리에이터 승인",
              isFreeAction: false,
              onClick: async (e) => {
                e.preventDefault();
                try {
                  const uuid = await selectedRow.map((item) => item.user.uuid);
                  console.log(uuid, "uuiduuiduuid확인중@@!!");
                  for (let i = 0; i < uuid.length; i++) {
                    creatorConfirm({
                      variables: {
                        user_uuid: uuid[i],
                        status: "ACCEPT",
                      },
                    });
                  }
                } catch (e) {
                  console.log(e);
                }
              },
            },
          ]}
        />
      </div>
    </div>
  );
};

export default UserList;
